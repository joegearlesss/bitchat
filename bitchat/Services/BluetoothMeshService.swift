...
class BluetoothMeshService: NSObject {
    ...
    // WiFi Bridge integration
    weak var bridgeService: WiFiBridgeService?
    private var isHybridModeEnabled = false
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
    ...
    // MARK: - WiFi Bridge Integration
    
    func setBridgeService(_ bridgeService: WiFiBridgeService) {
        self.bridgeService = bridgeService
        bridgeService.setMeshService(self)
    }
    ...
    func enableHybridMode(_ enabled: Bool) {
        ...
        if enabled {
            // Start bridge discovery and connection
            Task {
                await bridgeService?.connectToBridge()
            }
        } else {
    ...
    func handleBridgeMessage(_ packet: BitchatPacket) {
        // Process bridge message through existing pipeline
        messageQueue.async(flags: .barrier) { [weak self] in
            guard let self = self else { return }
            
            // Mark as bridge-originated message
            let senderID = String(data: packet.senderID.trimmingNullBytes(), encoding: .utf8) ?? "bridge"
            
            // Process through existing message handling
            self.handleReceivedPacket(packet, from: senderID, peripheral: nil)
        }
    }
    
    func sendViaBridge(_ packet: BitchatPacket) async throws {
        guard isHybridModeEnabled,
              let bridgeService = bridgeService,
              bridgeService.isConnected else {
    ...
    // Enhanced message sending with hybrid transport
    func sendMessageHybrid(_ packet: BitchatPacket) async {
        var bleSuccess = false
        var bridgeSuccess = false
        
        // Try BLE first (primary transport)
        if !connectedPeripherals.isEmpty {
            broadcastPacket(packet)
            bleSuccess = true
        }
        
        // Try bridge as fallback or supplement
        if isHybridModeEnabled && bridgeService?.isConnected == true {
            do {
                try await sendViaBridge(packet)
                bridgeSuccess = true
            } catch {
                print("Bridge send failed: \(error)")
            }
        }
        
        // Update bridge connection status
        DispatchQueue.main.async {
            self.bridgeConnected = self.bridgeService?.isConnected ?? false
        }
        
        // If neither transport worked, cache the message
        if !bleSuccess && !bridgeSuccess {
            cacheMessage(packet)
        }
    }
    
    // Check if we should use bridge for a specific message
    private func shouldUseBridge(for packet: BitchatPacket) -> Bool {
        guard isHybridModeEnabled else { return false }
        
        // Use bridge if no BLE peers available
        if connectedPeripherals.isEmpty {
            return true
        }
        
        // Use bridge for high-priority messages (redundancy)
        if packet.type == MessageType.privateMessage.rawValue {
            return true
        }
        
        // Use bridge for channel messages to extend reach
        if let payload = try? JSONDecoder().decode(MessagePayload.self, from: packet.payload),
           payload.content.hasPrefix("#") {
            return true
        }
    ...
    // Update the existing broadcastPacket method to support hybrid mode
    private func broadcastPacketHybrid(_ packet: BitchatPacket) {
        // Send via BLE
        broadcastPacket(packet)
        
        // Also send via bridge if appropriate
        if shouldUseBridge(for: packet) {
            Task {
                do {
                    try await sendViaBridge(packet)
                } catch {
                    print("Bridge broadcast failed: \(error)")
                }
            }
        }
    }
